  ...
  <div class='home'>
    <div class="w-full py-4 px-2" :class="[sisa < 0 ? 'bg-red-600' : 'bg-blue-600' ]">
      <div class="flex flex-wrap">
        <div class="md:w-1/3" v-if="editBudget">
          <input
          class='appearance-none w-12 block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500'
          id='grid-budget'
          type='number'
          placeholder='Budget'
          v-model="NewBudget"
        ...
        <h1 v-else @click="changeBudget" class="py-2 w-full md:w-1/3 block text-gray-200 font-bold md:w-1/3">Budget : {{ budget | currency }}</h1>
        <h1 class="py-2 w-full md:w-1/3 block font-bold text-gray-200" >Sisa : {{ sisa | currency }}</h1>
        <h1 class="py-2 w-full md:w-1/3 block font-bold text-gray-200">Pengeluaran : {{ pengeluaran | currency }}</h1>
      </div>
    </div>
    <div class='container mx-auto px-5 '>
      <div class="flex flex-wrap py-5">
        ...
        <div class="w-full md:w-1/2">
          <div class="relative w-32 float-right">
            <select v-model="sort" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
      ...
      <div class="flex flex-wrap ">
        <div class="flex-auto max-w-sm rounded block border border-gray-200 mx-2 my-2 overflow-hidden shadow-lg" v-for='(item, index) in countData' :key='index' >
          ...
          <div class="px-6 py-4">
            ...
            <p class="text-base text-left font-bold text-green-500">
          ...
          <div class="px-6 py-4">
            <button @click="confirmDelete(item.id)" class="inline-block bg-red-500 rounded-full px-3 py-1 text-sm font-semibold text-gray-200 mr-2">
              <svg class="fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
            </button>
            <button @click="editData(item.id)" class="inline-block bg-yellow-500 rounded-full px-3 py-1 text-sm font-semibold text-gray-200 mr-2">
              <svg class="fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18"><path d="M2 12.88V16h3.12L14 7.12 10.88 4 2 12.88zm14.76-8.51c.33-.33.33-.85 0-1.18l-1.95-1.95c-.33-.33-.85-.33-1.18 0L12 2.88 15.12 6l1.64-1.63z"/></svg>
            </button>
          </div>
        </div>
      </div>
    ...
    <modal name='form-input' :height="600" :draggable="true" @before-open="beforeOpen">
      ...
      <form @submit.prevent="saveData" class='w-full max-w-lg px-5 py-5 ml-10'>
        ...
        <div class="md:flex md:items-center">
          ...
          <div class="md:w-2/3">
            <button class="shadow float-right bg-blue-500 hover:bg-blue-700 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="submit">
              Save Data
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    saveData () {
      ...
      if (this.updateId !== '') {
        // this.items.splice(this.updateId, 1, {
        //   nama: this.nama,
